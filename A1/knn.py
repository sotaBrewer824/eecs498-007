...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.view(num_test, -1)
    for i in range(num_train):
      # Broadcast Mechanism: x_train[i] - x_test is equally to loop operations
      dists[i] = torch.sum((x_train[i] - x_test)**2, dim=1)
      dists[i] = dists[i].pow(0.5) 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train = x_train.view(num_train, -1) # shape: (num_train, C*H*W)
    x_test = x_test.view(num_test, -1) # shape: (num_test, C*H*W)
    x_train_square = torch.sum(x_train**2, dim=1) # shape: (num_train)
    x_test_square = torch.sum(x_test**2, dim=1) # shape: (num_test)
    train_cross_test = torch.mm(x_train, x_test.t()) # shape: (num_train, num_test)
    # Broadcast_Operations: x_train: (num_train) -> (num_train, 1)
    x_train_square = x_train_square.view(-1, 1)
    dists = x_train_square + x_test_square - 2 * train_cross_test
    dists = dists.pow(0.5)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    '''
    Function Name: torch.topk
    Description: Returns the k largest elements of the given input 
    ...
    Return:
      * values: Value Tensor of the topk method.
      * indices: Indices of the topk method.
    '''
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # Use the no-loop distance calculation
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        # Select a fold to be the test set
        x_train, x_test = torch.cat(x_train_folds[:i] 
              + x_train_folds[i + 1:], dim=0), x_train_folds[i]
        y_train, y_test = torch.cat(y_train_folds[:i] 
                     + y_train_folds[i + 1:]), y_train_folds[i]
