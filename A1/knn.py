...
def compute_distances_no_loops(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  x_tr, x_te = x_train.reshape(num_train, -1), x_test.reshape(num_test, -1)
  # x_tr: [num_train, C*H*W], x_te: [num_test, C*H*W] )
  x_tr_square = torch.sum(x_tr**2, dim=1)
  x_te_square = torch.sum(x_te**2, dim=1) # [num_test]
  x_tre = torch.mm(x_tr, x_te.t())
  dists = (x_tr_square.view(-1, 1) + x_te_square - 2 * x_tre)**0.5
  print(dists.shape)
