...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # X:128 x 3073 W: 3073 x 10
    scores = torch.mm(X, W) # scores: 128 x 10
    N, C = X.shape[0], W.shape[1] # N = 128, C = 10
    correct_class_score = scores[range(N), y].reshape(-1, 1) # 128 x 1
...
def softmax_loss_vectorized(
    ...
    minus_one_hot = torch.zeros_like(scores)
    minus_one_hot[range(N), y] = -1
    
    dW += minus_one_hot.t().mm(X).t()
