...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    # W: D x C, X: N x D
    scores = torch.mm(X, W) # scores: N x C
    N, C = X.shape[0], W.shape[1]
    # correct_class_score: N x 1
    correct_class_score = scores[range(N), y].reshape(-1, 1)
    zeros = torch.zeros_like(scores) # [0, 0, ...0], C x 1
    ...
    # Subtract the gradient contribution of the correct class position
    # Just remenber to do so, actually I do not trully understand
    # X: N x D
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    rand_samples = torch.randint(num_train, (batch_size, ))
    X_batch, y_batch = X[rand_samples], y[rand_samples]
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def softmax_loss_naive(
    ...
    # W: D x C, X: N x D
    for i in range(N):
        scores = W.t().mv(X[i]) # Same as SVM-Naive, (C, 1)
        # Stability
...
def softmax_loss_vectorized(
    ...
    loss = torch.log(exp_sum) - correct_class_score
    loss = loss.sum()
    ...
    minus_one_hot = torch.zeros_like(scores) # N x C
    minus_one_hot[range(N), y] = -1
    # X: N x D, W: D x C --> dW: D x C
    dW += torch.mm(X.t(), minus_one_hot)
