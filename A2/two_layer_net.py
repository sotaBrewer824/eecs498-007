...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    max_score_val = scores.max(dim=1).values # [5,]
    scores -= max_score_val.view(-1, 1)
    ...
    # computer the gradiensts by backpropagation
    # h1: N x H, grad_scores: N x C
    grads['W2'] = h1.t().mm(grad_scores)
    grads['W2'] += 2 * reg * W2
    
    grads['b2'] = torch.sum(grad_scores, dim=0) # the shape of b2 is [C, ]
    # need to sum over the first dimension of the tensor
