          ...
          for f in range(F):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    receptive_field = x_padded[n, :, h_start:h_start + HH, w_start:w_start + WW]
          ...
          for f in range(F):
            for h_out in range(H_out):
              for w_out in range(W_out):
                    h_start = h_out * stride
                    w_start = w_out * stride
                    receptive_field = x_padded[n, :, h_start:h_start + HH, 
                       w_start:w_start + WW]
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
          for c in range(C):
            for h_out in range(H_out):
              for w_out in range(W_out):
                h_start, h_end = h_out * stride, h_out * stride + pool_height
                w_start, w_end = w_out * stride, w_out * stride + pool_width
                recepitve_field = x[n, c, h_start: h_end, w_start: w_end]
                out[n, c, h_out, w_out] += torch.max(recepitve_field)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C):
            for h_out in range(H_out):
              for w_out in range(W_out):
                ...
                w_start, w_end = w_out * stride, w_out * stride + pool_width
                recepitve_field = x[n, c, h_start: h_end, w_start: w_end]
                mask = (recepitve_field == torch.max(recepitve_field))
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(2, self.num_layers):
          if weight_scale == 'kaiming':
            self.params[f'W{i}'] = kaiming_initializer(num_filters[i-2], num_filters[i-1], 
            K=3, relu=True, dtype=dtype, device=device)
          else:
          ...
          if weight_scale == 'kaiming':
            ...
            self.params[f'W{self.num_layers}'] = torch.normal(0.0, weight_scale, size=(dim_out, num_classes), \
             dtype=dtype, device=device)
