...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_flatten = x.reshape(x.shape[0], -1) # N x D
        out = torch.mm(x_flatten, w) + b # N x M
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        dw = torch.mm(x_flatten.t(), dout) # D x M
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        # First Layer: Linear_ReLU, Second Layer: Linear
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # First Layer
        self.params['W1'] = torch.normal(0.0, weight_scale, 
            size=(input_dim, hidden_dims[0]), dtype=dtype, device=device)
        self.params['b1'] = torch.zeros(hidden_dims[0], 
                     dtype=dtype, device=device)
        
        # input * h0 -> h0 * h1 -> ... -> h(L - 3) * h(L - 2) -> h(L - 2) * output
        for i in range(2, L):
          self.params[f'W{i}'] = torch.normal(0.0, weight_scale, 
               size=(hidden_dims[i-2], hidden_dims[i-1]), dtype=dtype, device=device)
          self.params[f'b{i}'] = torch.zeros(hidden_dims[i-1], dtype=dtype, device=device)
        
        self.params[f'W{L}'] = torch.normal(0.0, weight_scale, 
             size=(hidden_dims[L - 2], num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(2, L):
          h, cache0 = Linear_ReLU.forward(h, self.params[f'W{i}'], 
                              self.params[f'b{i}'])
          cache.append(cache0)
          if self.use_dropout:
            h, cache1 = Dropout.forward(h, self.dropout_param)
        ...
        scores, cache2 = Linear.forward(h, self.params[f'W{L}'], self.params[f'b{L}'])
        cache.append(cache2)
...
def rmsprop(w, dw, config=None):
    ...
    '''
    c = (1 - config['decay_rate']) * (dw**2) + config['decay_rate'] * config['cache'] 
    w -= (config['learning_rate'] * dw) / (c.sqrt() + config['epsilon'])
    config['cache'] = c
...
def adam(w, dw, config=None):
    ...
    config['t'] += 1 # t is your iteration counter going from 1 to infinity
    ...
    w -= config['learning_rate'] * mt / (vt.sqrt() + config['epsilon'])
