...
def rnn_step_backward(dnext_h, cache):
    ...
    d_hidden = (1 - torch.tanh(hidden) **2) * dnext_h # hidden: Wh, prev_h, Wx, x, b
    dx = torch.mm(d_hidden, Wx.t())
    dprev_h = torch.mm(d_hidden, Wh.t())
    dWx = torch.mm(x.t(), d_hidden)
    dWh = torch.mm(prev_h.t(), d_hidden)
    db = torch.sum(d_hidden, dim=0)
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    dtype, device = dh.dtype, dh.device
    for t in range(T-1, -1, -1):
      if t == T-1:
        dh_t = dh[:, t, :]
        dx_t, dh_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, cache[t])
        D = dx_t.shape[1]
        # initialize the dx, dWx, dWh, db tensors
        dx = torch.zeros(N, T, D, dtype=dtype, device=device)
        dWx = torch.zeros(D, H, dtype=dtype, device=device)
        dWh = torch.zeros(H, H, dtype=dtype, device=device)
        db = torch.zeros(H, dtype=dtype, device=device)
      else:
        dh_t += dh[:, t, :]
        dx_t, dh_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, cache[t])
      dx[:, t, :] = dx_t
...
class WordEmbedding(nn.Module):
  ...
  def __init__(self, vocab_size, embed_size,
      ...
      # Register parameters
      self.W_embed = nn.Parameter(torch.randn(vocab_size, embed_size,
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    loss = torch.nn.functional.cross_entropy(x.permute(0, 2, 1), y, \
